  ...
  return (
    <>
      ...
      <div className="flex max-h-[500px] flex-col gap-4 md:flex-row">
        <div className="relative order-1 w-full overflow-hidden rounded-md bg-neutral-800 p-4 md:order-2 md:aspect-video md:w-1/2">
          <Link className="h-1/5" href={`article/${article.id}`}>
            <h3 className="text-xl font-black hover:underline md:mb-4">
              {article.title}
            </h3>
          </Link>
          <div className="mt-4 hidden h-4/5 overflow-hidden text-ellipsis pb-6 md:block">
            {parse(article.body)}
          </div>
          <div className="absolute bottom-0 hidden h-3/4 w-full bg-gradient-to-t from-neutral-800 to-transparent md:block"></div>
        </div>
